...
void main() {
  ...
  function deriviative(a1, a2, p1, p2) {
    ...
    let da1 =
      ((6 / ml2) * (2 * p1 - 3 * cos12 * p2)) / (16 - 9 * cos12 * cos12);
    let da2 =
      ((6 / ml2) * (8 * p2 - 3 * cos12 * p1)) / (16 - 9 * cos12 * cos12);
  ...
  function pendulum({
    ...
    return {
      ...
      clone: function (conf) {
        if (!conf) conf = {};
        let cp2;
        if (p2 === 0.0) cp2 = Math.random() * 1e-12;
        else cp2 = p2 * (1 - Math.random() * 1e-10);
  ...
  (function () {
    ...
    function cb(t) {
      ...
      if (running)
        for (let i = 0; i < state.length; i++) state[i].step(dt / 1000.0);
      if (mode === "3d") {
        clear3d(gl);
        renderer.renderAll(state);
      } else {
        clear2d(ctx);
        for (let i = 0; i < state.length; i++) state[i].draw2d(ctx);
      }
      last = t;
  ...
  return (
    <div>
      <canvas ref={c2dRef} id="c2d"></canvas>
