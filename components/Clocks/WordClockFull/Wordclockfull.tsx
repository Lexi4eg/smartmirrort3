...
const WordclockFUll = (props: Props) => {
    ...
    function DotDiv({condition, style}: any) {
        return (
            <div
                className={` ${condition ? "" : "text-opacity-0"} ${style === "nightmode" ? "text-nightmode" : "text-white"}`}>
                ⬤
            </div>
        );
    }
    ...
    return (
        <div className={"w-screen h-screen flex flex-col items-center justify-center p-3"}>
            <div className="flex w-full flex-row justify-between text-md">
                <DotDiv condition={currentTime.getMinutes() % 5 >= 1} style={props.style}/>
                <DotDiv condition={currentTime.getMinutes() % 5 >= 2} style={props.style}/>
            </div>

            <div
                className={`w-full h-full px-5 justify-center items-center flex-col ${props.style === "nightmode" ? "text-red-950" : "text-slate-700"}`}>
            ...
            <div className="flex w-full flex-row justify-between">
                <DotDiv condition={currentTime.getMinutes() % 5 >= 3} style={props.style}/>
                <DotDiv condition={currentTime.getMinutes() % 5 >= 4} style={props.style}/>
            </div>
        </div>
    );
};
...
function Words(props: WordProps) {
    return (
        ...
        >
            <div className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl ${black_Ops_One.className}`}>{props.char}</div>
        </div>
    );
}

export default WordclockFUll;